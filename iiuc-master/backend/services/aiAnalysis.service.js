...
Create detailed notes with this EXACT JSON structure (no markdown, no code blocks):
...
Generate 3 topics minimum. Make content technical and detailed. Return ONLY the JSON object.`;
      ...
      console.log('First 300 chars of cleaned:', cleanContent.substring(0, 300));

      let notes;
      try {
        notes = JSON.parse(cleanContent);
        console.log('Successfully parsed JSON');
      } catch (parseError) {
        console.error('JSON Parse Error:', parseError.message);
        console.error('Failed content (first 500 chars):', cleanContent.substring(0, 500));

        // Try one more time with even more aggressive cleaning
        try {
          const ultraClean = cleanContent
            .replace(/[\u0000-\u001F\u007F-\u009F]/g, '') // Remove control characters
            .replace(/,(\s*[}\]])/g, '$1'); // Remove trailing commas

          notes = JSON.parse(ultraClean);
          console.log('Successfully parsed after ultra cleaning');
        } catch (secondError) {
          throw new Error(`Failed to parse AI response as JSON: ${parseError.message}. Content preview: ${cleanContent.substring(0, 200)}`);
        }
      }
      ...
      console.log('Notes generated successfully with', notes.topics?.length || 0, 'topics');
      return notes;
    } catch (error) {
...
Write a powerful, personalized summary highlighting their unique strengths. Use LaTeX for metrics (e.g., $95\\%$). No pronouns. Be specific and impactful.`;
      ...
      const response = await groq.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "Expert resume writer. Create personalized, compelling summaries. Use LaTeX for numbers/metrics. Be concise and powerful."
          },
          {
            role: "user",
            content: prompt
...
- Quantified results with LaTeX (e.g., "Improved by $60\\%$", "$O(n)$ complexity")
...
Return JSON array: ["bullet 1", "bullet 2", "bullet 3"]`;
        ...
        const response = await groq.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "Expert resume writer. Create powerful, quantified bullet points with LaTeX metrics. Return only JSON array."
            },
            {
              role: "user",
              content: prompt
  ...
  static async generateProfileRecommendations(profileData) {
    try {
      ...
      const prompt = `
        ...
        Provide recommendations in JSON format. Use LaTeX formatting for metrics and percentages in recommendations:
        {
          "linkedin": {
            "headline": "Recommended LinkedIn headline",
            "summary": "Recommended LinkedIn summary (3-4 sentences) with LaTeX for metrics",
      ...
      const response = await groq.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a career advisor and professional branding expert. Provide actionable recommendations with LaTeX formatting for metrics. Return only valid JSON."
          },
          {
            role: "user",
            content: prompt
