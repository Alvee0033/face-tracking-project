...
export default function RecruiterDashboardClient({ initialData }: { initialData?: any }) {
  ...
  const { data: dashboardData, isLoading, error: fetchError } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response: any = await apiClient.get('/jobs/recruiter/my-jobs?limit=5' as any)
                            ...
                            >
                              <td className="px-6 py-5">
                                <div className="flex flex-col">
                                  <span className="font-bold text-slate-900 group-hover:text-teal-600 transition-colors text-base">{job.title}</span>
                                  <span className="text-xs font-medium text-slate-500 bg-slate-100 w-fit px-2 py-0.5 rounded mt-1">{job.jobType || 'Full-time'}</span>
                                </div>
                              </td>
                              <td className="px-6 py-5">
