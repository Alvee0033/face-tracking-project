...
export default function RecruiterJobsClient({ initialData }: { initialData?: any }) {
  ...
  const { data: jobsData, isLoading: loading, error: queryError } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const response: any = await apiClient.get(`/jobs/recruiter/my-jobs${statusQuery}` as any)
                      ...
                      filteredJobs.map((job, index) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <div className="text-sm font-bold text-slate-800 group-hover:text-teal-700 transition-colors">{job.title}</div>
                            <div className="text-xs text-slate-500 mt-0.5 flex items-center gap-1 font-medium">
                              <span className="truncate max-w-[150px]">{job.location}, {job.country}</span>
                            </div>
                          </td>
                ...
                filteredJobs.map((job, index) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3 mb-4">
                      <div>
                        <h3 className="text-lg font-bold text-slate-900 leading-tight mb-1">{job.title}</h3>
                        <p className="text-sm text-slate-500 font-medium">{job.location}, {job.country}</p>
                      </div>
