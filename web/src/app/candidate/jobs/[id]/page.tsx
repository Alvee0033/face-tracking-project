...
export default function ViewJobPage() {
  ...
  const mapJob = (j: any) => {
    ...
    return {
      ...
      recruiter_profiles: j.recruiter_profiles || (j.recruiter ? {
        company_name: j.company || j.recruiter?.companyName || j.recruiter?.fullName,
        industry: j.industry || 'Technology',
        company_size: j.companySize || '11-50 employees'
      } : {
        company_name: j.company || j.company_name || 'Company'
      }),
      job_skills: Array.isArray(j.requiredSkills) ? j.requiredSkills.map((s: string) => ({ skill_name: s })) : (j.job_skills || [])
    }
  }
  ...
  const checkIfApplied = async () => {
    try {
      ...
      const alreadyApplied = applications.some(
        (app: any) => app.jobId === params.id || app.jobId === String(params.id)
      )
  ...
  const handleSubmitApplication = async (e: React.FormEvent) => {
    ...
    try {
      ...
      sessionStorage.removeItem(`applications_${params.id}`)
      setTimeout(async () => {
